...
void Fsm::check_transit_and_get_update_queue(Vector<Fsm*> *&update_queue) {
    ...
    for (auto &&transition : _current_state->_transition_list) {
        ...
        if (transition->can_transit()) {
            ...
            // 切换并进入新状态
            _current_state = transition->get_to_state();
            // TODO:: 是否应该在进入新状态完成后再发射信号？
            // 暂定未进入
            _hfsm->transited(_current_state, transition->get_to_state());
    ...
    if (!_current_state->is_exited()) { // 当前状态仍在运行
        ...
        if (_current_state->_fsm && _current_state->_fsm->_running) {
            // 当前状态具有内嵌状态机且正在运行
            // 尝试对其进行转换并获取更新队列
            _current_state->_fsm->check_transit_and_get_update_queue(
                update_queue);
        }
    } else {
        // 已停止運行
...
void Fsm::_interpolate_state(const Array &old_state, const Array &new_state,
                             real_t weight) {
    uint64_t idx = 1;
    for (auto &&s : _state_list) {
        idx++;
...
Array Fsm::_predict_remote_input(const Array &previous_input,
                                 int64_t ticks_since_real_input) {
    Array ret;
    for (size_t i = 0; i < _state_list.size(); i++) {
        ret.push_back(_state_list.get(i)->predict_remote_input(
            previous_input[i], ticks_since_real_input));
    }
    return ret;
}
