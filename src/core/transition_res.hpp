#ifndef TRANSITION_RES_H
#define TRANSITION_RES_H
...
private:
    // 共有属性
    Ref<StateRes> _from_res = nullptr;
    Ref<StateRes> _to_res = nullptr;
    uint8_t _type = TRANSITION_TYPE_AUTO;

    // Auto
    uint8_t _auto_mode = AUTO_TRANSIT_MODE_DELAY_TIMER;
    uint64_t _auto_delay_msec = 1000; // 延迟时间
    uint64_t _auto_times = 5;         // 重复次数
    ...
    TypedArray<VariableExpressionRes> _variable_expression_res_list;
