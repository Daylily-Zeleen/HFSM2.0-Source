...
bool TransitionRes::_set(const StringName &p_name, const Variant &p_property) {
    if (p_name == StringName("type")) {
        ...
        return true;
    } else if (p_name == StringName("update_times") ||
               p_name == StringName("physics_update_times")) {
...
void TransitionRes::_get_property_list(List<PropertyInfo> *p_list) const {
    p_list->push_back(PropertyInfo(Variant::STRING, "from", PROPERTY_HINT_NONE,
                                   "", PROPERTY_USAGE_EDITOR));
    p_list->push_back(PropertyInfo(Variant::STRING, "to", PROPERTY_HINT_NONE,
                                   "", PROPERTY_USAGE_EDITOR));
    p_list->push_back(PropertyInfo(Variant::INT, "type", PROPERTY_HINT_ENUM,
                                   "Script,HFSMVariable,Expression,Auto"));
    switch (_type) {
    case TRANSITION_TYPE_SCRIPT: {
        p_list->push_back(PropertyInfo(Variant::OBJECT, "transition_script",
                                       PROPERTY_HINT_RESOURCE_TYPE, "Script", PROPERTY_USAGE_STORAGE, "Script"));
    } break;
    case TRANSITION_TYPE_VARIABLE: {
        p_list->push_back(PropertyInfo(Variant::BOOL, "and_mode"));

        auto typed_VariableExpressionRes_array_hint_string =
            String("{0}/{1}:VariableExpressionRes")
                .format(
                    Array::make(Variant::OBJECT, PROPERTY_HINT_RESOURCE_TYPE));

        p_list->push_back(
            PropertyInfo(Variant::ARRAY, "variable_expression_res_list",
                         PROPERTY_HINT_TYPE_STRING,
                         typed_VariableExpressionRes_array_hint_string));
    } break;
    case TRANSITION_TYPE_EXPRESSION: {
        p_list->push_back(PropertyInfo(Variant::STRING, "expression_text",
                                       PROPERTY_HINT_MULTILINE_TEXT));
        p_list->push_back(PropertyInfo(Variant::STRING, "expression_comment",
                                       PROPERTY_HINT_MULTILINE_TEXT));
    } break;
    case TRANSITION_TYPE_AUTO: {
        auto p = PropertyInfo(
            Variant::INT, "auto_mode", PROPERTY_HINT_ENUM,
            "Delay Timer,Fsm Exit,Manual,Update Times,Physics Update Times");
        p_list->push_back(p);
        switch (_auto_mode) {
        case AUTO_TRANSIT_MODE_DELAY_TIMER: {
            p_list->push_back(PropertyInfo(Variant::INT, "delay_msec",
                                           PROPERTY_HINT_RANGE,
                                           "0,2147483647,1,or_greater"));
        } break;
        case AUTO_TRANSIT_MODE_UPDATE_TIMES: {
            p_list->push_back(PropertyInfo(Variant::INT, "update_times",
                                           PROPERTY_HINT_RANGE,
                                           "0,2147483647,1,or_greater"));
        } break;
        case AUTO_TRANSIT_MODE_PHYSICS_UPDATE_TIMES: {
            p_list->push_back(PropertyInfo(Variant::INT, "physics_update_times",
                                           PROPERTY_HINT_RANGE,
                                           "0,2147483647,1,or_greater"));
        } break;
...
void TransitionRes::_bind_methods() {
    // 通用
    ClassDB::bind_method(D_METHOD("get_from_state_res"),
                         &TransitionRes::get_from_state_res);
    ClassDB::bind_method(D_METHOD("set_from_state_res", "from_state_res"),
                         &TransitionRes::set_from_state_res);
    ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "from_state_res",
                              PROPERTY_HINT_RESOURCE_TYPE, String("StateRes"),
                              PROPERTY_USAGE_STORAGE, StringName("StateRes")),
                 "set_from_state_res", "get_from_state_res");

    ClassDB::bind_method(D_METHOD("get_to_state_res"),
                         &TransitionRes::get_to_state_res);
    ClassDB::bind_method(D_METHOD("set_to_state_res", "to_state_res"),
                         &TransitionRes::set_to_state_res);
    ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "to_state_res",
                              PROPERTY_HINT_RESOURCE_TYPE, String("StateRes"),
                              PROPERTY_USAGE_STORAGE, StringName("StateRes")),
                 "set_to_state_res", "get_to_state_res");

    ClassDB::bind_method(D_METHOD("get_type"), &TransitionRes::get_type);
    ClassDB::bind_method(D_METHOD("set_type", "type"),
                         &TransitionRes::set_type);
    // ADD_PROPERTY(PropertyInfo(Variant::INT, "type", PROPERTY_HINT_ENUM,
    //                           "Script,HFSMVariable,Expression,Auto"),
    //              "set_type", "get_type");

    // Auto
    ClassDB::bind_method(D_METHOD("get_auto_mode"),
                         &TransitionRes::get_auto_mode);
    ClassDB::bind_method(D_METHOD("set_auto_mode", "auto_mode"),
                         &TransitionRes::set_auto_mode);

    ClassDB::bind_method(D_METHOD("get_auto_delay_msec"),
                         &TransitionRes::get_auto_delay_msec);
    ClassDB::bind_method(D_METHOD("set_auto_delay_msec", "delay_time"),
                         &TransitionRes::set_auto_delay_msec);

    ClassDB::bind_method(D_METHOD("get_auto_times"),
                         &TransitionRes::get_auto_times);
    ClassDB::bind_method(D_METHOD("set_auto_times", "auto_times"),
                         &TransitionRes::set_auto_times);
    // 表达式
    ClassDB::bind_method(D_METHOD("get_expression_text"),
                         &TransitionRes::get_expression_text);
    ClassDB::bind_method(D_METHOD("set_expression_text", "expression_text"),
                         &TransitionRes::set_expression_text);

    ClassDB::bind_method(D_METHOD("get_expression_comment"),
                         &TransitionRes::get_expression_comment);
    ClassDB::bind_method(
        D_METHOD("set_expression_comment", "expression_comment"),
        &TransitionRes::set_expression_comment);

    // 变量表达式
    ClassDB::bind_method(D_METHOD("is_and_mode"), &TransitionRes::is_and_mode);
    ClassDB::bind_method(D_METHOD("set_and_mode", "and_mode"),
                         &TransitionRes::set_and_mode);

    ClassDB::bind_method(D_METHOD("get_variable_expression_res_list"),
                         &TransitionRes::get_variable_expression_res_list);
    ClassDB::bind_method(D_METHOD("set_variable_expression_res_list",
                                  "variable_expression_res_list"),
                         &TransitionRes::set_variable_expression_res_list);
    ...
    //  脚本
    ClassDB::bind_method(D_METHOD("get_transition_script"),
                         &TransitionRes::get_transition_script);
    ClassDB::bind_method(D_METHOD("set_transition_script", "transition_script"),
                         &TransitionRes::set_transition_script);
...
String TransitionRes::get_expression_comment() const {
    return _expression_comment;
}
...
void TransitionRes::set_variable_expression_res_list(
    Array variable_expression_res_list) {
...
Ref<Script> TransitionRes::get_transition_script() const {
    return _transition_script;
}

TransitionBase *TransitionRes::create_transition(HFSM *hfsm,
    ...
    case TRANSITION_TYPE_VARIABLE: {
        ...
        for (size_t i = 0; i < _variable_expression_res_list.size(); i++) {
            Ref<VariableExpressionRes> variable_expression_res =
                _variable_expression_res_list[i];
            ...
            case VariableExpression::ExpressionType::UNION_TRIGGER:
                vt->_union_triggers.append(
                    static_cast<UnionTriggerExpression *>(ve));
                break;
            case VariableExpression::ExpressionType::SOLO_TRIGGER:
                vt->_solo_triggers.append(
                    static_cast<SoloTriggerExpression *>(ve));
                break;
            default:
                break;
            }
