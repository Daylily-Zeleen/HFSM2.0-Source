...
void FsmRes::add_state_res(const Ref<StateRes> &state_res) {
	...
	do {
		ununique = false;
		for (auto i = 0; i < _state_res_list.size(); i++) {
			Ref<StateRes> sr = _state_res_list[i];
			StringName name = state_res->get_state_name();
			StringName be_check = sr->get_state_name();
			if (name == be_check) {
				name = String("@") + String(name);
				state_res->call("set_state_name", name);
				ununique = true;
				break;
			}
		}
...
void FsmRes::add_transition_res(const Ref<TransitionRes> &transition_res) {
...
void FsmRes::remove_transition_res(const Ref<TransitionRes> &transition_res) {
	if (_transition_res_list.find(transition_res) >= 0) {
